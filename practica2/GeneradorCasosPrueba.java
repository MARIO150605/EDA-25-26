 ...
 * VERSION 2: Garantiza que no hay conexiones entre la misma persona
 * VERSION 3: OPTIMIZADA RESPECTO AL TIEMPO 18/11/2025
 * 
 * @author César Vaca
 */
public class GeneradorCasosPrueba {
    ...
    public static class Conexion {
        ...
        public Conexion(int usr1, int usr2) { this.usr1 = usr1; this.usr2 = usr2; }

        @Override
        public int hashCode() { return usr1 ^ usr2; }
        ...
        @Override
        public String toString() { return String.format("%d %d", usr1, usr2); }
    }
    ...
    public static List<Conexion> generaCaso(int n, Random rnd) {
        ...
        int[] usr = new int[n];
        for(int i = 0; i < n; i++) { usr[i] = i + 10000000; }
        // Algoritmo de Fisher-Yates (shuffle)
        for(int i = n-1; i > 0; i--) {
            int j = rnd.nextInt(i+1);
            int tmp = usr[j]; usr[j] = usr[i]; usr[i] = tmp;
        }
        ...
        for(int i1: inds) { // Rango [i0,i1)
            if(i1 < i0+2) { continue; }  // No se actualiza i0 aposta.
    ...
    public static void main(String[] args) throws IOException {
        ...
        if(nomfich.length() == 0) { nomfich = "test"+n+".txt"; }
